...
export default function Hero() {
  ...
  const containerVariants = { hidden: { opacity: 0, y: 30 }, visible: { opacity: 1, y: 0, transition: { duration: 2, ease: 'easeOut' } } };
  const textVariants = { hidden: { opacity: 0, scale: 1.2 }, visible: { opacity: 1, scale: 1, transition: { duration: 2, ease: 'easeOut' } } };
  const flashVariants = { hidden: { textShadow: '0 0 0 rgba(255,255,255,0)' }, visible: { textShadow: ['0 0 8px rgba(255,255,255,0.5)','0 0 18px rgba(255,255,255,1)','0 0 8px rgba(255,255,255,0.5)'], transition: { repeat: Infinity, repeatType: 'loop', duration: 2, ease: 'easeInOut', delay: 2 } } };
  const buttonVariants = { hidden: { scale: 0 }, visible: { scale: 1, transition: { duration: 1, ease: 'easeOut' } }, hover: { scale: 1.1, transition: { duration: 0.3, ease: 'easeInOut' } } };

  return (
    <motion.section initial="hidden" animate="visible" variants={containerVariants} style={{ position: 'relative', width: '100%', height: '100svh', minHeight: '400px' }}>
