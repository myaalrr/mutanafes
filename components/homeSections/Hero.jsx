...
export default function Hero() {
  ...
  // State لمتابعة المستخدم
  const [user, setUser] = useState(null);

  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      setUser(currentUser); // إذا مسجل دخول، currentUser موجود
    });
    return () => unsubscribe();
  }, []);
      ...
      >
        ...
        <p
          ...
          style={{
            ...
            opacity: 0.3,
            transition: 'width 0.08s ease-out, filter 0.08s ease-out, opacity 0.08s ease-out',
            color: '#fff',
            marginTop: '10px',
        ...
        {/* أزرار تسجيل الدخول / إنشاء حساب تظهر فقط إذا المستخدم غير مسجل */}
        {!user && (
          <motion.div
            initial="hidden"
            animate="visible"
